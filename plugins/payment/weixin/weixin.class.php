...
class weixin extends Model{    
    ...
    public function  __construct() {   
        ...
        $paymentPlugin = Db::name('plugin')->where(['code'=>'weixin','type'=>'payment'])->find(); // 找到微信支付插件的配置
    ...
    function get_code($order, $config_value){       
            $notify_url = 'http://'.$_SERVER['HTTP_HOST'].'/home/payment/notifyUrl/pay_code/weixin'; // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
            $input = new WxPayUnifiedOrder();
            $input->SetBody("Ybcms商品"); // 商品描述
            ...
            $input->SetProduct_id("1234567890"); // 商品ID trade_type=NATIVE，此参数必传。此id为二维码中包含的商品ID，商户自行定义。
            $notify = new NativePay();
            $result = $notify->GetPayUrl($input); // 获取生成二维码的地址
            $url2 = $result["code_url"];
            return '<img alt="模式二扫码支付" src="'.url('Home/Index/qr_code').'?data='.urlencode($url2).'" style="width:110px;height:110px;"/>';        
    }    
    /**
     * 服务器点对点响应操作给支付接口方调用
     * 
    ...
    function getJSAPI($order){
    	if(stripos($order['order_sn'],'recharge') !== false){
    		$go_url = url('mobile/user/points',['type'=>'recharge']);
    		$back_url = url('mobile/user/recharge',['order_id'=>$order['order_id']]);
    	}else{
    		$go_url = url('mobile/user/order_detail',['id'=>$order['order_id']]);
    		$back_url = url('mobile/cart/cart4',['order_id'=>$order['order_id']]);
    	}
        //①、获取用户openid
        $tools = new JsApiPay();
        ...
        $input->SetNotify_url('http://'.$_SERVER['HTTP_HOST'].'/mobile/payment/notifyUrl/pay_code/weixin');
		...
		function(res){
			...
			if(res.err_msg == "get_brand_wcpay_request:ok") {
			    location.href='$go_url';
			 	...
			 	alert('错误代码：'+res.err_code+'错误信息：'+res.err_desc+'错误说明：'+res.err_msg);
