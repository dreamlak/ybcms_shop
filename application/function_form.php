 ...
 * @param string 	$msg		说明
 */
function tpl_upfile($id,$name,$value='',$auid=0,$uuid=0,$placeholder='文件URL,可直接填写文件远程地址',$msg=''){
	$str="
		<div class=\"input-group\">
			<input type=\"text\" name=\"{$name}\" id=\"{$id}\" value=\"{$value}\" placeholder=\"{$placeholder}\" class=\"form-control\">
			<span class=\"input-group-btn\">
				<button class=\"btn btn-default\" type=\"button\" id=\"upload_{$id}\">上传文件</button>
			</span>
		</div>
	";
	if($msg!='')$str.="<span class=\"help-block\">{$msg}</span>\n";
        ...
        $('#upload_{$id}').click(function(){
	        require(['bootstrap','think'], function(){
				var parame={};
					...
					parame.filenum=1;
					parame.ext='rar,zip,doc,docx,xls,xlsx,ppt,pptx';
					parame.upfilesize=1024*1024*50;
					parame.ismark=0,
					parame.adminid={$auid};
					parame.userid={$uuid};
