...
use app\index\logic\GoodsLogic;
use think\AjaxPage;
...
class Goods extends Base {
    ...
    */ 
    public function goodsInfo(){
        //form表单提交
        $goodsLogic = new \app\index\logic\GoodsLogic();
        //echo '=='.input('get.id');
        $goods_id = input("id/d");
       ...
       //C('URL_MODEL',0);
        ...
        if($filter_cat_id)        {
            ...
            foreach($cateArr as $k => $v)            {
                $tmp['id'] = $v['id'];
                $cateArr[$k]['href'] = url("/index/Goods/search",$tmp);                            
            }                
        }                        
        // 过滤帅选的结果集里面找商品        
        if($brand_id || $price){
    ...
    public function goodsConsult(){
        ...
        if (true !== $result) {
            $this->error($result, url('/index/Goods/goodsInfo',array('id'=>$goods_id)));             
            exit;
        }                
        ...
        $this->success('咨询已提交!', url('/index/Goods/goodsInfo',['id'=>$goods_id]));
    }
