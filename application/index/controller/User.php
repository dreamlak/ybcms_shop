...
class User extends Base{
    ...
    public function login(){
        ...
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U("index/User/index");
    ...
    public function do_login(){
    	...
    	if($res['status'] == 1){
    		if(strstr(input('referurl'),'index/index/index')||input('referurl')==''){
    			$res['url'] =  url('index/User/index');
    		}else{
    ...
    public function reg(){
        ...
        if(request()->isPost()||request()->isAjax()){
            ...
            //$this->success($data['msg'],url('index/User/index'));
			return ['status'=>1,'msg'=>$data['msg'],'url'=>url('index/User/index')];
            exit;
        }
        ...
        if(request()->isPost()||request()->isAjax()){
            ...
            if($info['sender'] == $email && $info['code'] == $code){
                ...
                if(!$userLogic->update_email_mobile($email,$this->userid))
                    $this->error('邮箱已存在');
                $this->success('绑定成功',U('index/User/index'));
                exit;
            }
            $this->error('邮箱验证码不匹配');
        }
    ...
    public function password(){
        ...
        if($user['tel'] == ''&& $user['email'] == '')
            $this->error('请先绑定手机或邮箱',url('index/User/safetycenter'));
        if(request()->isPost()||request()->isAjax()){
            $userLogic = new UsersLogic();
    ...
    public function set_pwd(){
    	...
    	$logic = new UsersLogic();
    	if(empty($check)){
            $this->redirect('index/User/forget_pwd');
    	}elseif($check['is_check']==0){
    		$this->error('验证码还未验证通过',url('index/User/forget_pwd'));
    	}
		//修改密码
    	if(request()->isPost()||request()->isAjax()){
    		...
    		if($password2 != $password){
    			$this->error('两次密码不一致',url('index/User/forget_pwd'));
    		}
    		if($check['is_check']==1){
                ...
                $this->redirect('index/User/finished');
    		}else{
    			$this->error('验证码还未验证通过',U('index/User/forget_pwd'));
    		}
    	}
    	return $this->fetch();
    }
    ...
    public function return_goods(){
        ...
        if(!empty($return_goods)){
            $this->success('已经提交过退货申请!',url('index/User/return_goods_info',array('id'=>$return_goods['id'])));
            exit;
        }       
        if(request()->isPost()||request()->isAjax()){
            ...
            $this->success('申请成功,客服第一时间会帮你处理',url('index/User/order_list'));
            exit;
        }
