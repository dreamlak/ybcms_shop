$(function() {
	...
	//推荐品牌logo鼠标经过效果
	$(".store-wall2-list li").hover(function(){
...
$(function() {
	if ($(".floor")) {
		...
		$.each($('.floor'), function(i, v) {
			var fnum = $.trim($(v).find('.SZY-FLOOR-NAME').text());
			var short_name = $.trim($(v).find('.SZY-FLOOR-NAME').data('name'));
			if (short_name == '')
				short_name = fnum;
		...
		for (var i = 0; i < $(".floor").length; i++) {
			arr.push(parseInt($(".floor").eq(i).offset().top) + 0)
		}
...
$(document).ready(function(){ 
	var headHeight=250;  //这个高度其实有更好的办法的。使用者根据自己的需要可以手工调整。
