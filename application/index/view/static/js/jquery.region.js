 ...
 */
(function($) {
	//跟据定位的城市编码，组编成（省、市、县）编码数组
	function resolveRegionCode(region_code) {
		var codes = [];
		var region_code_arr=[];
		//拆分
		var adcodes = region_code.split("");
		for(var i = 0; i < adcodes.length; i++){
			region_code_arr.push(adcodes[i] + "" + adcodes[i + 1]);
			i++;
		}
		//组合
		for(var j=0;j<region_code_arr.length;j++){
			if(j==0){
				codes[j]=region_code_arr[0]+'0000';
			}else if(j==1){
				codes[j]=region_code_arr[0]+region_code_arr[1]+'00';
			}else if(j==2){
				codes[j]=region_code_arr[0]+region_code_arr[1]+region_code_arr[2];
			}
		}
		//console.log(codes);
		return codes;
	...
	$.fn.regionselector = function(settings) {
		var defaults = {
			...
			load: function(region_code, value_mode, target) {
				...
				$.get(this.url, {region_code:region_code}, function(result) {
					//console.log(result);
					if (result.code != 0) {
						return;
					}
			...
			reload: function() {
				...
				/*if (settings.url == null) {
					if (settings.sale_scope == true) {
						settings.url = '/common/Area/getRegionJson.html';
					} else {
						settings.url = '/common/Area/getRegionJson.html';
					}
				}*/
	...
	$.fn.regionchooser = function(settings) {
		var defaults = {
			...
			load: function(region_code, value_mode, target) {
				...
				if (target != null) {
					return $.get(settings.url, {region_code: region_code}, function(result) {
			...
			render: function(list, value_mode, target) {
				...
				if (list == undefined) {
					...
					$(container).on("click", ".region-items a", function() {
						...
						var region_code = $(this).data("region-code");
						settings.value=region_code.toString();
				...
				if (target != undefined || target != null || value_mode) {
					if($(target).parent().data('region-level')==3){
						is_last = true;
					}
					//console.log(is_last);
