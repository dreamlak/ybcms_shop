...
(function($) {
	...
	$.cart = {
		...
		quickBuy: function(id, number, options) {
			...
			$.post('_data.php', data, function(result) {
				if (result.code == 0) {
					$.go(result.data);
				} else {
	...
	$.compare = {
		...
		// 清空对比商品
		clear: function() {
			$.post('_data.php', {}, function(result) {
				if (result.code == 0) {
					// 重置
					$.sidebar.comparebox.load();
				} else {
		...
		getGoodsList: function(goods_ids, callback) {
			...
			$.get('_data.php', data, function(result) {
