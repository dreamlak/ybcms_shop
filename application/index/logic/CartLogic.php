...
class CartLogic extends Model{
     ...
     */
    function addCart($goods_id,$goods_num,$goods_spec,$session_id,$userid = 0){       
        $goods = Db::name('goods')->where("goods_id", $goods_id)->cache(true,CACHE_TIME)->find(); // 找出这个商品
        $specGoodsPriceList = Db::name('spec_goods_price')->where("goods_id", $goods_id)->cache(true,CACHE_TIME)->column("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码
        ...
        if($goods['prom_type'] > 0 && $userid == 0){
            return array('status'=>-101,'msg'=>'购买活动商品必须先登录','result'=>'');
        }
        //限时抢购 不能超过购买数量        
        if($goods['prom_type'] == 1){
            $flash_sale = Db::name('flash_sale')->where(['id'=>$goods['prom_id'],'start_time'=>['<',time()],'end_time'=>['>',time()],'goods_num'=>['>','buy_num']])->find(); // 限时抢购活动
            if($flash_sale){
        ...
        if($userid > 0){
            $where .= " and (session_id = :session_id or userid = :userid) ";
            $cart_bind['session_id'] = $session_id;
            $cart_bind['userid'] = $userid;
        } else{
        ...
        if($goods['prom_type'] > 0){            
            $prom = get_goods_promotion($goods_id,$userid);
            $price = $prom['price'];
       ...
       if($catr_goods){          
           // 如果购物车的已有数量加上 这次要购买的数量  大于  库存输  则不再增加数量
            ...
            $cart_count = cart_goods_num($userid,$session_id); // 查找购物车数量 
    ...
    function cartList($user = array() , $session_id = '', $selected = 0,$mode =0){                   
        $where = " 1 = 1 ";
        $bind = array();
        if($user['userid']){// 如果用户已经登录则按照用户id查询
    ...
    function cart_freight2($shipping_code, $province, $city, $district, $weight){
        ...
        $shipping_area_id = Db::name('shipping_area_region')->where("shipping_area_id in (select shipping_area_id from  " . config('database.prefix') . "shipping_area where shipping_code = :shipping_code) and region_id = :region_id")->bind(['shipping_code'=>$shipping_code,'region_id'=>$district])->value('shipping_area_id');
        // 先根据市区找 shipping_area_id
        if ($shipping_area_id == false){
            $shipping_area_id = Db::name('shipping_area_region')->where("shipping_area_id in (select shipping_area_id from  " . config('database.prefix') . "shipping_area where shipping_code = :shipping_code) and region_id = :region_id")->bind(['shipping_code'=>$shipping_code,'region_id'=>$city])->value('shipping_area_id');
		}
        // 市区找不到 根据省份找shipping_area_id
        if ($shipping_area_id == false){
            $shipping_area_id = Db::name('shipping_area_region')->where("shipping_area_id in (select shipping_area_id from  " . config('database.prefix') . "shipping_area where shipping_code = :shipping_code) and region_id = :region_id")->bind(['shipping_code'=>$shipping_code,'region_id'=>$province])->value('shipping_area_id');
        }
        // 省份找不到 找默认配置全国的物流费
        if ($shipping_area_id == false) {
            ...
            $shipping_area_id = Db::name("shipping_area")->where(['shipping_code'=>$shipping_code,'is_default'=>1])->value('shipping_area_id');
        }
        if ($shipping_area_id == false){
            return 0;
        }
        /// 找到了 shipping_area_id  找config
        $shipping_config = Db::name('shipping_area')->where("shipping_area_id", $shipping_area_id)->value('config');
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    public function getCouponMoney($userid, $coupon_id,$mode){
        if($coupon_id == 0){
            if($mode == 1) return 0;    
            return array('status'=>1,'msg'=>'','result'=>0);            
        }        
        $couponlist = Db::name('coupon_list')->where("uid", $userid)->where('id', $coupon_id)->find(); // 获取用户的优惠券
        if(empty($couponlist)) {
            if($mode == 1) return 0;    
            return array('status'=>1,'msg'=>'','result'=>0);
        }            
    ...
    public function getCouponMoneyByCode($couponCode,$order_momey){
        $couponlist = Db::name('coupon_list')->where("code", $couponCode)->find(); // 获取用户的优惠券
        if(empty($couponlist)) 
     ...
     */
    public function addOrder($userid,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note=''){
        //仿制灌水 1天只能下 50 单  // select * from `tp_order` where userid = 1  and order_sn like '20151217%' 
        $order_count = Db::name('Order')->where("userid",$userid)->where('order_sn', 'like', date('Ymd')."%")->count(); // 查找购物车商品总数量
        if($order_count >= 50){ 
            return array('status'=>-9,'msg'=>'一天只能下50个订单','result'=>'');            
		}
        //0插入订单 order
        $address = Db::name('member_address')->where("id", $address_id)->find();
        $shipping = Db::name('plugin')->where("code", $shipping_code)->cache(true,CACHE_TIME)->find();
        $data = array(
                ...
                'integral'         =>($car_price['pointsFee'] * config('config.point_rate')), //'使用积分',
        ...
        // 1插入order_goods 表
        $cartList = Db::name('cart')->where(['userid'=>$userid,'selected'=>1])->select();
        foreach($cartList as $key => $val){
           $goods = Db::name('goods')->where("goods_id", $val['goods_id'])->cache(true,CACHE_TIME)->find();
        ...
        if($coupon_id > 0){
        	...
        	Db::name('coupon_list')->where("id", $coupon_id)->update($data3);
                $cid = Db::name('coupon_list')->where("id", $coupon_id)->value('cid');
                Db::name('coupon')->where("id", $cid)->setInc('use_num'); // 优惠券的使用数量加一
        }
        // 3 扣除积分 扣除余额
        if($car_price['pointsFee']>0){
        	Db::name('member')->where("userid", $userid)->setDec('mypoints',($car_price['pointsFee'] * config('config.point_rate'))); // 消费积分
		}
        if($car_price['balance']>0){
        	Db::name('member')->where("userid", $userid)->setDec('mymoney',$car_price['balance']); // 抵扣余额
        }
        // 4 删除已提交订单商品
        Db::name('cart')->where(['userid' => $userid,'selected' => 1])->delete();
      
        // 5 记录log 日志
        $data4['userid'] = $userid;
        $data4['mymoney'] = -$car_price['balance'];
        $data4['mypoints'] = -($car_price['pointsFee'] * config('config.point_rate'));
        $data4['change_time'] = time();
        $data4['desc'] = '下单消费';
        $data4['order_sn'] = $order['order_sn'];
        $data4['order_id'] = $order_id;    
        // 如果使用了积分或者余额才记录
        ($data4['mymoney'] || $data4['mypoints']) && Db::name("member_account_log")->insert($data4);
        
        //分销开关全局
        $distribut_switch = config('config.distribut_switch');
        ...
        if($user['oauth']== 'weixin' && $user['openid']!=''){
            $jssdk = new \app\mobile\logic\Jssdk(config('config.wx_appid'),config('config.wx_appsecret'));
            $wx_content = "你刚刚下了一笔订单:{$order['order_sn']} 尽快支付,过期失效!";
            $jssdk->push_msg($user['openid'],$wx_content);
        }
    	//用户下单, 发送短信给商家
    	$sender = config("config.site_mobile");
    	$params = array('consignee'=>$order['consignee'],'phone'=>$order['mobile']);
    	$resp = sendSms("4", $sender, $params);
     ...
     * $mode 0  返回数组形式  1 直接返回result
     */
    public function cart_count($userid,$mode = 0){
        $count = Db::name('cart')->where(['userid' => $userid , 'selected' => 1])->count();
   ...
   public function get_group_buy_price($goods_id,$mode=0){
       $group_buy = Db::name('group_buy')->where(['goods_id' => $goods_id,'start_time'=>['<=',time()],'end_time'=>['>=',time()]])->find(); // 找出这个商品
       if(empty($group_buy))       
            return 0;
    ...
    * @param type $userid
    */
   public function login_cart_handle($session_id,$userid){
	   if(empty($session_id) || empty($userid))
	     return false;
        // 登录后将购物车的商品的 userid 改为当前登录的id            
        Db::name('cart')->where("session_id", $session_id)->update(array('userid'=>$userid));
                
        // 查找购物车两件完全相同的商品
        $cart_id_arr = query("select id from `__PREFIX__cart` where userid = $userid group by  goods_id,spec_key having count(goods_id) > 1");
     ...
     * @return array
     */
    public function addPreSellOrder($userid,$address_id,$shipping_code,$invoice_title,$act_id,$pre_sell_price){
        // 仿制灌水 1天只能下 50 单
        $order_count = Db::name('order')->where("userid= $userid and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
        if($order_count >= 50){
            return array('status'=>-9,'msg'=>'一天只能下50个订单','result'=>'');
        }
        $address = Db::name('Member_address')->where(array('id' => $address_id))->find();
        ...
        logOrder($order_id,'您提交了订单，请等待系统确认','提交订单',$userid);
