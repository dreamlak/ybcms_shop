...
function ajaxGetRegion()
{
	...
	$.ajax({
		type : "get",
		dataType:'json',
		url:"/index.php/Home/Goods/getRegion",
...
function ajaxDispatching(region_id) {
	...
	if(typeof(goods_id) != 'undefined' && region_id!= ''){
		$.ajax({
			...
			url: "/index.php/Home/Goods/dispatching",
			success: function (data) {
				if (data.status == 1) {
...
//                    $.getJSONP("https://d.jd.com/area/get?callback=getAreaListcallback&fid="+cityId);
				$.getJSONP("/index.php/Home/Goods/region/callback/getAreaListcallback/fid/"+cityId);
			}
...
//                    $.getJSONP("https://d.jd.com/area/get?callback=getAreaListcallback&fid="+areaId);
				$.getJSONP("/index.php/Home/Goods/region/callback/getAreaListcallback/fid/"+areaId);
			}
