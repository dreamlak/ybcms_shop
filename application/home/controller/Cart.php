...
class Cart extends Base {
    ...
    public function cart2(){        
        if($this->userid == 0){
            $this->error('请先登陆',url('Home/User/login'));
		}
    ...
    public function cart4(){
        ...
        if($order['pay_status'] == 1){            
            $order_detail_url = url("Home/User/order_detail",array('id'=>$order_id));
    ...
    public function pre_sell_cart()
    {
        ...
        if(empty($goods_num)){
            $this->error('购买商品数量不能为0', url('Home/Activity/pre_sell', array('act_id' => $act_id)));
        }
        if($this->userid == 0){
            $this->error('请先登陆');
        }
