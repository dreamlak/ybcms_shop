...
class GoodsLogic extends Model{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action, $mode = 0){
        ...
        $list_brand = Db::name('brand')->where("id in(select brand_id from " .config('database.prefix') . "goods where brand_id > 0 and goods_id in(:goods_id_str))")->bind(['goods_id_str'=>$goods_id_str])->limit('30')->select();  //where("goods_id in($goods_id_str)")->select();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $spec_key = query("select group_concat(`key` separator  '_') as `key` from __PREFIX__spec_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        ...
        $goods_attribute = Db::name('goods_attribute')->where("attr_index = 1")->column('attr_id,attr_name,attr_index');
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
    ...
    public function commentStatistics($goods_id){
        $c1 = Db::name('goods_comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(4,5)")->bind(['goods_id'=>$goods_id])->count(); // 好评
        $c2 = Db::name('goods_comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(3)")->bind(['goods_id'=>$goods_id])->count(); // 中评
        $c3 = Db::name('goods_comment')->where("is_show = 1 and  goods_id = :goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(1,2)")->bind(['goods_id'=>$goods_id])->count(); // 差评
     ...
     * @return type
     */
    public function collect_goods($userid, $goods_id){
        if (!is_numeric($userid) || $userid <= 0) return array('status' => -1, 'msg' => '必须登录后才能收藏', 'result' => array());
        //$count = Db::name('Goods')->where("goods_id = $goods_id")->count();
        //if($count == 0)  return array('status'=>-2,'msg'=>'收藏的商品不存在','result'=>array());
        $count = Db::name('goods_collect')->where("userid",$userid)->where("goods_id", $goods_id)->count();
        if ($count > 0) return array('status' => -3, 'msg' => '商品已收藏', 'result' => array());
        Db::name('goods_collect')->insert(['goods_id'=>$goods_id,'userid'=>$userid,'add_time'=>time()]);
    ...
    public function get_spec($goods_id){
        ...
        $filter_spec = array();
        if($keys) {
            $specImage = Db::name('spec_image')->where(['goods_id'=>$goods_id,'src'=>['<>','']])->column("spec_image_id,src");// 规格对应的 图片表， 例如颜色
    ...
    function getGoodsIdByAttr($attr)
    {
        ...
        $c = count($attr_id) - 1;
        if ($c > 0)
            $arr = Db::name('goods_attr')->where($where)->bind($bind)->group('goods_id')->having("count(goods_id) > $c")->column("goods_id", true);  //select * from   `tp_goods_attr` where attr_id in(59,80) and attr_value in('直板','翻盖','蓝牙4.0') group by goods_id having count(goods_id) > 1
        else
            $arr = Db::name('goods_attr')->where($where)->bind($bind)->column("goods_id", true); // 如果只有一个条件不再进行分组查询
    ...
    function getRegionList(){
        $parent_region = Db::name('areas')->where('pid',0)->field('id,name')->cache(true)->select();
        $ip_location = array();
        $city_location = array();
        foreach($parent_region as $key=>$val){
            $c = Db::name('areas')->field('id,name')->where('pid',$parent_region[$key]['id'])->order('id asc')->cache(true)->select();
    ...
    function getParentRegionList($cid){
        //$pids = '';
        $pids = array();
        $parent_id =  Db::name('areas')->where(array('id'=>$cid))->value('pid');
        if($parent_id != 0){
            //$pids .= $parent_id;
    ...
    function getGoodsDispatching($goods_id,$region_id){
        ...
        if($goods['is_free_shipping']){
            $return_data['msg'] = '有货';
            $return_data['result'] = array(['shipping_name'=>'包邮','freight'=>0]);
            return $return_data;
        }
        ...
        $find_shipping_area_id = Db::name('shipping_area_region')->where(array('region_id'=>array('in',$parent_region_id)))->group('shipping_area_id')->column('shipping_area_id');
        ...
        $goods_shipping = Db::name('')
            ->table(config('database.prefix').'shipping_area_region ar')
    ...
    */
    function get_filter_selected($sel ,$cat_id = 1){
        ...
        //看全部
        if($sel == 'all'){
            $arrid = $Goods->column('goods_id', true);
        }else{
            $arrid = $Goods->where($where)->column('goods_id', true);
        }
        return $arrid;
    }
}
