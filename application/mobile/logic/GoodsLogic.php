...
class GoodsLogic extends Model{
    ...
    public function refresh_cat($id){            
        ...
        if($cat['parent_id_path'] == ''){
            ...
            execute("UPDATE __PREFIX__goods_category AS a ,__PREFIX__goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = :id",['id'=>$id]);
    ...
    public function getAttrInput($goods_id,$type_id){
        ...
        $GoodsAttribute = Db::name('goods_attribute');
    ...
    public function saveGoodsAttr($goods_id,$goods_type){  
        ...
        $Goods = Db::name("goods");
        $old_goods_type = $Goods->where('goods_id', $goods_id)->value('goods_type');
    ...
    public function getSpecInput($goods_id, $spec_arr){
        ...
        foreach ($spec_arr_sort as $key =>$val){
     	...
     	$spec_arr2 = combineDika($spec_arr2); //  获取 规格的 笛卡尔积                 
      	...
      	$specItem = Db::name('spec_item')->getField('id,item');//规格项
       	$keySpecGoodsPrice = Db::name('spec_goods_price')->where('goods_id', $goods_id)->column('key,key_name,price,store_count,sku');//规格项
