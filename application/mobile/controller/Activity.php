    ...
    */ 
    public function group(){
        ...
        if($keys){
             ...
             $filter_spec2 = query($sql,['keys'=>$keys]);
    ...
    public function group_list(){
        $istype =input('get.type');
        //以最新新品排序
        if($istype == 'new'){
            $orderby = 'start_time desc';
        }
    	$count =  Db::name('group_buy')->where(time()." >= start_time and ".time()." <= end_time ")->count();// 查询满足要求的总记录数
        $pagesize = config('paginate.list_rows');  //每页显示数
    	$Page = new Page($count,$pagesize); // 实例化分页类 传入总记录数和每页显示的记录数
    	$show = $Page->show();  // 分页显示输出
    	$this->assign('page',$show);    // 赋值分页输出
        $list = Db::name('group_buy')->where(time()." >= start_time and ".time()." <= end_time ")->order($orderby)->limit($Page->firstRow.','.$Page->listRows)->select();   // 找出这个商品
    ...
    public function promote_goods(){
        ...
        $pagesize = config('paginate.list_rows');  //每页显示数
        $Page  = new Page($count,$pagesize); //分页类
        $promote = Db::name('prom_goods')->field('id,name,start_time,end_time,prom_img')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();    //查询活动列表
    ...
    public function seckill_list(){
        ...
        $count1 = query("select count(g.goods_id) as count from __PREFIX__goods as g inner join __PREFIX__flash_sale as f on g.goods_id = f.goods_id where start_time = $start_time and end_time = $end_time");     //查询符合要求的总记录数
        $pagesize = config('paginate.list_rows');  //每页分页数
        $count = $count1[0]['count'];    //统计数量
        $Page = new Page($count ,$pagesize);    //分页类
        $seckill_list=query("select f.goods_id,f.title,f.price,f.order_num,f.goods_num,g.goods_name,g.market_price from __PREFIX__goods as g inner join __PREFIX__flash_sale as f on g.goods_id = f.goods_id where start_time = $start_time and end_time = $end_time limit ".$Page->firstRow.",".$Page->listRows."");     //获取商品
