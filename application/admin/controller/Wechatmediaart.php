...
class Wechatmediaart extends AdminBase{
	...
	public function add(){
		if(request()->isPost() || request()->isAjax()){
			...
			if($mediaArr['status']==1){
				...
				$data['thumb_media_url']=$mediaArr['thumb_media_url'];
			}
	...
	public function edit(){
		if(request()->isPost() || request()->isAjax()){
			...
			//如果图片有更新，则重新上传素才库
			$msg='';
			$thumb_media_id=Db::name('wx_mediaart')->where(['artid'=>input('artid')])->value('thumb_media_id');
			if($data['oldthumb']!=$data['thumb']||$thumb_media_id==''){
				$mediaArr=model('Wechatmediaart')->upThumbMedia($data['thumb']);
				if($mediaArr['status']==1){
					$data['thumb_media_id']=$mediaArr['thumb_media_id'];
					$data['thumb_media_url']=$mediaArr['thumb_media_url'];
			...
			if($rs!==false){
				...
				return ['status'=>1,'msg'=>'编辑图文成功!'.$msg];
			}else{
				return ['status'=>0,'msg'=>'编辑图文败!'.$msg];
			}
		}else{
			$artid=input('artid');
	...
	//ajax上传图片到微信服务器获取URL
	public function ajaxGetWxmediaURL(){
		$imgfile=input('imgfile');
		return model('Wechatmediaart')->getMediaUrl($imgfile);
	}
	//内容批量转换图片到微信服务器获取URL(数据测试暂时不用)
	public function getContent($artid){
		$content=Db::name('wx_mediaart')->where('artid',$artid)->value('content');
		$re=optArtcons($content);
		dump($re);die;
	}
}
?>