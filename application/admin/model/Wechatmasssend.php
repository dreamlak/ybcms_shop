...
class Wechatmasssend extends Base{
	...
	//上传临时素材（有效期为3天）经过多次验证，群发要的是临时素材ID
	public function getMediaID($file){
		if($file=='') return ['status'=>0,'msg'=>'图文ID参数错误!'];
		$file=thumb($file,400,400,1);
		$path=str_replace('\\','/',$_SERVER['DOCUMENT_ROOT']).$file;
		$data=['media'=>'@'.$path];
		
		$media = & load_wechat('Media');
		$result = $media->uploadMedia($data,'thumb');//$type可选有图片image、语音voice、视频video、缩略图thumb

		//处理执行的结果
		if($result===FALSE){
		    // 接口失败的处理
		    return ['status'=>0,'msg'=>'上传临时素材到微信服务器：'.get_wechat_error($media->errCode)];
		}
	...
	public function getArtArr($artid){
		...
		$data[0]['thumb_media_id']=$this->getMediaID($indexArts['thumb']);
		$data[0]['author']=$indexArts['author'];
		$data[0]['title']=$indexArts['title'];
		if($indexArts['url']==''){
			$url=$httphost.url('mobile/Wechat/artshow',['artid'=>$indexArts['artid']]);
		}else{
			$url=$indexArts['url'];
		}
		...
		if(count($subArts)>0){
			foreach($subArts as $k=>$v){
				...
				$subData[$k]['thumb_media_id']=$this->getMediaID($v['thumb']);
