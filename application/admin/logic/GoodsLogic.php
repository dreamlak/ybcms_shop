...
class GoodsLogic extends Model{
    ...
    public function refresh_cat($id){            
        ...
        {
            ($cat['parent_id'] == 0) && execute("UPDATE __PREFIX__goods_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类               
            execute("UPDATE __PREFIX__goods_category AS a ,__PREFIX__goods_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");                
        ...
        execute("UPDATE `__PREFIX__goods_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");        
    }    
        ...
        foreach($attributeList as $key => $val){                                                                        
            ...
            foreach($curAttrVal as $k =>$v){                                        
                ...
                if($val['attr_input_type'] == 0){
                    $str .= "<input type='text' value='".($goods_id ? $v['attr_value'] : $val['attr_values'])."' name='attr_{$val['attr_id']}[]' class=\"form-control\" />";
                }
                // 从下面的列表中选择（一行代表一个可选值）
                ...
                if($val['attr_input_type'] == 2){
                    $str .= "<textarea cols='40' rows='3' name='attr_{$val['attr_id']}[]' class=\"form-control\">".($goods_id ? $v['attr_value'] : $val['attr_values'])."</textarea>";
                }                                                        
                $str .= "</td></tr>";
            }                        
        }        
        ...
        foreach($post as $k => $v)
        {
           ...
           {                      
               ...
               if(array_key_exists($tmp_key , $old_goods_attr)){// 如果这个属性 原来就存在
                   ...
                   $GoodsAttr->insert(array('goods_id'=>$goods_id,'attr_id'=>$attr_id,'attr_value'=>$v2,'attr_price'=>$attr_price));                       
               }
    ...
    public function getSpecInput($goods_id, $spec_arr){
        ...
        $keySpecGoodsPrice = Db::name('Spec_goods_price')->where('goods_id',$goods_id)->column('key,key_name,price,store_count,bar_code,sku');//规格项
        ...
        $str .="<td><b>价格</b></td>
       	...
       	foreach ($spec_arr2 as $k => $v){
            ...
            $str .="<td><input name='item[$item_key][price]' value='{$keySpecGoodsPrice[$item_key]['price']}' class=\"form-control\" onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
            $str .="<td><input name='item[$item_key][store_count]' value='{$keySpecGoodsPrice[$item_key]['store_count']}' class=\"form-control\" onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")'/></td>";            
            $str .="<td><input name='item[$item_key][sku]' value='{$keySpecGoodsPrice[$item_key]['sku']}' class=\"form-control\" />
    ...
    function getSortBrands(){
        ...
        $goodsCategoryArr = Db::name('goods_category')->where("level = 1")->column('id,name');
        $nameList = array();
        ...
        array_multisort($nameList,SORT_STRING,SORT_ASC,$brandList);//排序
        return $brandList;
    }    
    ...
    function getSortCategory(){
        $categoryList =  model("GoodsCategory")->field('id,name,parent_id,level')->select();
        $nameList = array();
    ...
    function savePreSell($data)
    {
        ...
        if (array_key_exists('shut_down', $data) && $data['shut_down'] == 1) {
            //结束活动
            if (!empty($data['id'])) {
                $pre_sell_shut_down = Db::name('goods_activity')->where(array('act_id' => $data['id']))->update(array('end_time' => time()));
                if ($pre_sell_shut_down !== false) {
                    return array('msg' => '编辑预售活动成功。', 'status' => 1, 'url' => url('Promotion/pre_sell_info', array('id' => $data['id'])));
                } else {
                    return array('msg' => '编辑预售活动失败。', 'status' => 0, 'url' => url('Promotion/pre_sell_info', array('id' => $data['id'])));
                }
            } else {
                return array('msg' => '未指定需结束的预售商品', 'status' => 0, 'url' => url('Promotion/pre_sell_list'));
            }
        }
        if($data['deposit'] > 0){
            ...
            if($data['retainage_start'] === false || $data['retainage_end'] === false){
                return array('msg'=>'您输入了一个无效的时间，请选择尾款支付时间！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
            if ($data['retainage_start'] > $data['retainage_end']) {
                return array('msg'=>'您输入了一个无效的时间，尾款结束支付时间不能早于尾款开始支付时间！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
            if ($data['start_time'] > $data['retainage_start'] && $data['retainage_start']) {
                return array('msg'=>'您输入了一个无效的时间，尾款开始支付时间不能早于活动结束时间！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
        }else{
        ...
        if ($data['start_time'] > $data['end_time']) {
            return array('msg'=>'您输入了一个无效的时间，活动结束时间不能早于活动开始时间！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if (empty($data['restrict_amount']) || $data['restrict_amount'] <= 0) {
            return array('msg'=>'您输入了一个无效的预售库存，预售库存要大于0！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if ($data['restrict_amount'] < max($data['ladder_amount'])) {
            return array('msg'=>'您没有输入有效的价格阶梯，预定最多人数不能大于预售库存！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if ($data['deposit'] > min($data['ladder_price'])) {
            return array('msg'=>'定金不能大于阶梯价格！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if($price_ladder[0]['amount'] <= 0 ||$price_ladder[0]['price'] <= 0){
            return array('msg'=>'您没有输入有效的价格阶梯！','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        $ext_info = array(
            'sale_price' => 0,
        ...
        if($goods_main['store_count'] < $ext_info['restrict_amount']){
            return array('msg'=>'预售库存大于商品库存','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
        }
        if (empty($data['id'])) {
            //添加一个活动的判断
            ...
            if($is_have_goods){
                if($is_have_goods['is_finished'] == 0){
                    return array('msg'=>'该商品已经参加了预售活动，不能再次参与预售活动','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
                }
            }
            if($goods_main['prom_type'] != 0){
                return array('msg'=>'该商品已经参加了其他活动，不能参与预售活动','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
        }
        if(!empty($data['id']) && !empty($data['goods_id'])){
            ...
            if($goods_activity['start_time'] < time() && $goods_activity['goods_id'] != $data['goods_id']){
                return array('msg'=>'预售活动已经开始不能更改预售商品','status'=>0, 'url'=>url('Promotion/pre_sell_info', array('id' => $data['id'])));
            }
        }
        ...
        if (empty($data['id'])) {
            ...
            M('goods')->where(array('goods_id'=>$data['goods_id']))->update($goods_save_data);
            $r = Db::name('goods_activity')->add($add_data);
            adminLog("管理员添加商品预售活动 " . $data['goods_name']);
            if ($r !== false) {
                return array('msg'=>'添加预售商品活动成功','status'=>1, 'url'=>url('Promotion/pre_sell_list'));
            } else {
                return array('msg'=>'添加预售商品活动失败','status'=>0, 'url'=>url('Promotion/pre_sell_list'));
            }
        } else {
            $save_data = array(
                'act_name' => $data['goods_name'],
            ...
            $r = Db::name('goods_activity')->where(array('act_id' => $data['id'], 'act_type' => 1))->update($save_data);
            if ($r !== false) {
                return array('msg'=>'编辑预售商品活动成功','status'=>1, 'url'=>url('Promotion/pre_sell_list'));
            } else {
                return array('msg'=>'编辑预售商品活动失败','status'=>0, 'url'=>url('Promotion/pre_sell_list'));
            }
        }
    }
}