...
class OrderLogic extends Model{
    ...
    public function getOrderButton($order){
         ...
         */
    	$os = $order['order_status'];//订单状态 0待确认 1已确认 2已收货 3已取消 4已完成 5已作废
    	$ss = $order['shipping_status'];//发货状态 0未发货 1已发货 2部分发货
    	$ps = $order['pay_status'];//支付状态 0未支付 1已支付
        $btn = array();
        if($order['pay_code'] == 'cod') {//货到付款
        	if($os == 0 && $ss == 0){
        		$btn['confirm'] = '确认订单';
        	}elseif($os == 1 && $ss == 0 ){
        		$btn['delivery'] = '去发货';
    ...
    function order_pay_cancel($order_id){
    	...
    	foreach($orderGoodsArr as $key => $val){
    		...
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品的总数量
    			Db::name('spec_goods_price')->where("goods_id = {$val['goods_id']} and `key` = '{$val['spec_key']}'")->setInc('store_count',$val['goods_num']);
    	...
    	// 记录订单操作日志
    	logOrder($order['order_id'],'订单取消付款','付款取消',$order['userid']);
    	//分销设置
    	Db::name('rebate_log')->where("order_id = {$order['order_id']}")->update(array('status'=>0));
    }
    ...
    public function getAddressName($p=0,$c=0,$d=0){
        $p = Db::name('areas')->where(array('id'=>$p))->field('name')->find();
        $c = Db::name('areas')->where(array('id'=>$c))->field('name')->find();
        $d = Db::name('areas')->where(array('id'=>$d))->field('name')->find();
    ...
    function delOrder($order_id){
    	$a = Db::name('order')->where('order_id',$order_id)->delete();
    	$b = Db::name('order_goods')->where('order_id',$order_id)->delete();
		$c = Db::name('order_action')->where('order_id',$order_id)->delete();
		$d = Db::name('goods_comment')->where('order_id',$order_id)->delete();
    	return $a && $b;
    }
}