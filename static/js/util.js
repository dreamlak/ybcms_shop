(function(window) {
	...
	//单张图片上传
	util.images=function(parame){
		//if(parame.length==0) return false;
		var ismark=parame.ismark,
			inputid=parame.inputid,
			btn=parame.btn,
			imgid=parame.imgid,
		...
		require(['webuploader'],function(WebUploader){
			uploader = WebUploader.create({
			    ...
			    // 文件接收服务端。
			    server: Think.U('common/Fileupload/fileupload','uptypes=images&ismark='+ismark+'&adminid='+adminid+'&userid='+userid),
			    // 选择文件的按钮。可选。
			    // 内部根据当前运行是创建，可能是input元素，也可能是flash.
			    pick:'#'+btn,
		        accept:{title:'images',extensions:'gif,jpg,jpeg,bmp,png'},
		        //fileNumLimit:1, //上传文件总数
	 ...
	 * 打开文件上传窗口
	 * @params parame	参数组｛｝
	 * @params parame.filetype		文件类型
	 * @params parame.filenum		可上传文件数量
	 * @params parame.ext			可上传扩展名
	 * @params parame.upfilesize	单个可上传大小
	 * @params parame.inputid		接收反回文件地址的表单ID
	 * @params parame.ismark		是否水印
	 * @params parame.adminid		管理员
	 * @params parame.userid		用户
	 */
	util.upfile=function(parame,callback){
		if(!parame) parame = {};
		var optname = 'UploadFiles';//+parseInt(10*Math.random());
		var modalobj = $('#' + optname);
		if(modalobj.length == 0) {
			$(document.body).append('<div id="' + optname + '" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true"></div>');
			modalobj = $('#' + optname);
		}
		var typename;
		if(parame.filetype=='images'){
			typename='图片';
		}else if(parame.filetype=='file'){
			typename='文件';
		}else if(parame.filetype=='video'){
			typename='视频';
		}else if(parame.filetype=='audio'){
			typename='音频';
		}
		html = 
			...
			'			<h4 style="margin:0">文件'+typename+'</h4>'+
			'		</div>'+
			'		<div class="modal-body">正在加载...</div>'+
			'	</div>'+
			'</div>';
		modalobj.html(html);
		$.post(Think.U('common/Fileupload/index'),parame,function(data){
	        modalobj.find('.modal-body').html(data)
	    },'html');
		//modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'630px'});
		modalobj.find('.modal-body').css({'height':'500px','overflow':'scroll','overflow-x':'hidden','overflow-y':'auto'});
		modalobj.modal('show');
		modalobj.on('shown.bs.modal', function () {
			require(['uploadfile'], function(uploadfile) {
		        uploadfile.files(parame,function(images){
		        	if(images.length>0){
						if($.isFunction(callback)){
							modalobj.modal('hide');
		        			modalobj.remove();
	...
	//移出对像
	util.removeobj=function(id){
		$('#'+id).remove();
	}
	
	//单个文件上传
	util.oneupfile=function(paramef,obj){
		//if(paramef.length==0) return false;
		var filetype=paramef.filetype,
			inputid=paramef.inputid,
			btn=paramef.btn,
			ext=paramef.ext,
			allsize=paramef.allsize,
			showsize=paramef.showsize,
			adminid=paramef.adminid,
			userid=paramef.userid;
		...
		require(['webuploader'],function(WebUploader){
			obj = WebUploader.create({
			    ...
			    swf: '/static/js/webuploader/js/Uploader.swf',
			    // 文件接收服务端。
			    server: Think.U('common/Fileupload/fileupload','uptypes='+filetype+'&ismark=0&adminid='+adminid+'&userid='+userid),
			    // 选择文件的按钮。可选。
			    // 内部根据当前运行是创建，可能是input元素，也可能是flash.
			    pick:'#'+btn,
		        accept:{title:filetype,extensions:ext},
		        fileVal:'file',//设置文件上传域的name
		        chunked:true,//是否要分片处理大文件上传
		        chunkSize: 1024 * 1024 * 5,//分片 默认大小为5M
		        fileSingleSizeLimit:allsize,//验证单个文件大小是否超出限制, 超出则不允许加入队列
		        duplicate:true,//重复
		        auto:true
			});
			obj.on('uploadSuccess', function(file,response){
			    $('#'+inputid).val(response.url);
			    $('#'+showsize).text(response.size); 
			});
			obj.on('beforeFileQueued', function(file){
			   	obj.onError = function( code ) {
			    	if(code=='Q_EXCEED_NUM_LIMIT'){
			    		alert('添加的文件数量超出');return false;
			    	}else if(code=='Q_EXCEED_SIZE_LIMIT'){
			    		alert('添加的文件总大小超出');return false;
	...
	util.emotion = function(elm, target, callback) {
		require(['jquery.caret', 'bootstrap', 'css!/static/js/emotions/emotions.css'],function($){
			$(function() {
	...
	util.loading = function() {
		...
		if(modalobj.length == 0) {
			...
			html = 
				...
				'		<img style="width:48px; height:48px; margin-top:100px;" src="/static/images/public/loading.gif" title="正在努力加载...">'+
				'	</div>'+
				'</div>';
			modalobj.html(html);
		}
	...
	//弹出提交页面
	util.submitpage = function(url,title,form,width,height){
		var footer ='<button type="button" class="btn btn-primary" id="modalsubmit">确认提交</button>'+
					'<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog(title,[url],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':width});
		modalobj.find('.modal-body').css({'height':height,'overflow-y':'scroll'});
		modalobj.modal('show');
		modalobj.find('button#modalsubmit').on('click', function(){
	...
	util.map = function(val, callback){
		require(['map'], function(BMap){
			...
			if(modalobj.length == 0) {
				...
				var label = new BMap.Label("请您移动此标记，选择您的坐标！",{offset:new BMap.Size(20,-10)});
				/*label.setStyle({
					color : "red",
					fontSize : "12px",
					width:"200px",
	...
	util.iconBrowser = function(callback){
		...
		var modalobj = util.dialog('请选择图标',['/data/icon.html'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'70%'});
		modalobj.find('.modal-body').css({'height':'600px','overflow-y':'scroll'});
		modalobj.modal('show');
	...
	util.emojiBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog('请选择表情',['/data/biaoping.html'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'70%'});
	...
	*/
	...
	util.city=function(parame){
		var ids = parame.id.split(",");
		var name = parame.name.split(",");
		var vals = parame.vals.split(",");
		var idslen=ids.length;
		for(var i=0;i<idslen;i++){
			$('#'+ids[i+1]).empty().css('display','none');
		}
		if(idslen>0){
			$('#'+ids[0]).change(function(){
				$('#'+ids[1]).empty().removeAttr('style');
				$('#'+ids[1]).empty().html('<option value="">选择'+name[1]+'</option>');
		    	var value=$(this).val();
		    	if(value=='') return false;
		    	$.post(Think.U('common/area/ajaxArea'),{pcode:value},function(data){
		            if (data.status==1){
		            	var list=data.list;
		            	for(var i=0;i<list.length;i++){  
				            $('#'+ids[1]).append("<option value='"+list[i]['code']+"'>"+list[i]['name']+"</option>");  
				        }
		            }
		        });
			});
		...
		if(idslen>2){
			$('#'+ids[1]).change(function(){
				$('#'+ids[2]).empty().removeAttr('style');
				$('#'+ids[2]).empty().html('<option value="">选择'+name[2]+'</option>');
		    	var value=$(this).val();
		    	if(value=='') return false;
		    	$.post(Think.U('common/area/ajaxArea'),{pcode:value},function(data){
		            if (data.status==1){
		            	var list=data.list;
		            	for(var i=0;i<list.length;i++){  
				            $('#'+ids[2]).append("<option value='"+list[i]['code']+"'>"+list[i]['name']+"</option>");  
				        }
		            }
		        });
			});
		...
		if(idslen>3){
			$('#'+ids[2]).change(function(){
				$('#'+ids[3]).empty().removeAttr('style');
				$('#'+ids[3]).empty().html('<option value="">选择'+name[3]+'</option>');
		    	var value=$(this).val();
		    	if(value=='') return false;
		    	$.post(Think.U('common/area/ajaxArea'),{pcode:value},function(data){
		            if (data.status==1){
		            	var list=data.list;
		            	for(var i=0;i<list.length;i++){  
				            $('#'+ids[3]).append("<option value='"+list[i]['code']+"'>"+list[i]['name']+"</option>");  
				        }
		            }
		        });
			});
